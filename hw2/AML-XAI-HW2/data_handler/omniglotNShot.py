...
class OmniglotNShot:
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz, trainer='maml'):
        """
        ...
        # Reptile needs all data points for N classes during meta training to make mini-batch 
        if self.trainer == 'reptile':
            flag = True
        else:
            flag = False
        self.datasets_cache = {"train": self.load_data_cache(self.datasets["train"], flag),  # current epoch data cached
    ...
    def load_data_cache(self, data_pack, flag=False):
        """
        ...
        if flag:
            k_query = 20 - self.k_shot
        else:
            k_query = self.k_query
        setsz = self.k_shot * self.n_way
        querysz = k_query * self.n_way
        data_cache = []
        ...
        for sample in range(10):  # num of episodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                for j, cur_class in enumerate(selected_cls):
                    selected_img = np.random.choice(20, self.k_shot + k_query, False)
                ...
                perm = np.random.permutation(self.n_way * k_query)
                x_qry = np.array(x_qry).reshape(self.n_way * k_query, 1, self.resize, self.resize)[perm]
                y_qry = np.array(y_qry).reshape(self.n_way * k_query)[perm]
    ...
    def next(self, mode='train'):
        ...
        if self.indexes[mode] >= len(self.datasets_cache[mode]):
            self.indexes[mode] = 0
            if mode == 'train' and self.trainer == 'reptile':
                flag = True
            else:
                flag = False
            self.datasets_cache[mode] = self.load_data_cache(self.datasets[mode], flag)
